...
var exports = module.exports = function(file, context) {
  ...
  if (file.isText() && exports.settings.useLint) {
    // promise = pipe(file, 'lint', true);
    promise = promise.then(function () {
      return pipe(file, 'lint', true);
    });
  }
...
function process(file, context) {
  ...
  return pipe(file, 'parser').then(function () {
    return pipe(file, 'preprocessor');
  }).then(function () {
    return pipe(file, 'standard');
  }).then(function () {
    return postStandard(file, context);
  }).then(function () {
    return pipe(file, 'postprocessor');
  }).then(function () {
    return pipe(file, 'optimizer');
  }).then(function () {
...
function pipe(file, type, keep) {
  ...
  if (processors.length) {
    ...
    var callback = function(processor, settings, key) {
      ...
      var promise = processor(content, file, settings);
    ...
    processors.forEach(function(processor, index) {
      ...
      if (typeof processor === 'function') {
        ...
        promise = promise.then(function () {
          return callback(processor, settings, key);
        });
      } else {
